...
- `js()` shares a global scope across calls in the same `bh` execution. Wrap extraction in an IIFE — `const`/`let` at top level will collide on re-run. (General bh gotcha, but bites here because you'll iterate selectors.)
...
bh < agent-workspace/domain-skills/claude-ai/extract-share-transcript.py
```

The script reads both settings from environment variables; standard input carries the Python source, so environment variables are the cleanest configuration channel.
